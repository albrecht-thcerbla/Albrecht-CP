...
struct SegmentTree {
  ...
  int64_t get(int X, int lx, int rx, int x1, int x2, int y1, int y2) {
    if (x1 > rx || x2 < lx) return 0;
    if (x1 <= lx && x2 >= rx) return get_y(X, 1, 0, n - 1, y1, y2);
    int mx = (lx + rx) >> 1;
    return get(X << 1, lx, mx, x1, x2, y1, y2) + get(X << 1 | 1, mx + 1, rx, x1, x2, y1, y2);
  }
};
int32_t main() {
  ...
  while (q--) {
    ...
    x1--; x2--; y1--; y2--;
    cout << t.get(1, 0, n - 1, y1, y2, x1, x2) << '\n';
  }
  return 0;
}